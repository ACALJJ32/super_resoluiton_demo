...
from utils.utils import make_layer, flow_warp, ResidualBlockNoBN, ResidualBlocksWithInputConv, SPyNet, PixelShufflePack


class BasicVSRPlusPlus_Gauss(nn.Module):
    ...
    def __init__(self,
        ...
        # dft feature extraction
        self.dft_extractor = DftFeatureExtractor(mid_channels=mid_channels, num_blocks=5, with_gauss=True)
    ...
    def propagate(self, feats, flows, module_name):
        ...
        for i, idx in enumerate(frame_idx):
            ...
            if i > 0:
                ...
                # dft feature
                feat_prop = self.dft_extractor(feat_prop)
    ...
    def forward(self, lqs):
        ...
        # visualize for optical flow
        flows_out = flows_backward[0].clone()
        ...
        return self.upsample(lqs, feats), flows_out
...
class DftFeatureExtractor(nn.Module):
    def __init__(self, mid_channels=64, num_blocks=10, with_gauss=True, guass_keys = 2.0):
        super().__init__()
        # Gauss 
        self.with_gauss = with_gauss
        self.guass_key = guass_keys
        self.register_parameter('gauss_mat', nn.Parameter(torch.ones(1, 1, mid_channels, mid_channels)))
    ...
    def forward(self, lr):
        ...
        x_proj = (2 * math.pi * x)
        x_proj = x_proj.view(-1, c, h*w).permute(0, 2, 1)

        if self.with_gauss:
            gauss_key =  self.gauss_mat * self.guass_key
            gauss_b = self.modulation(gauss_key)
            x_proj = torch.matmul(x_proj, gauss_b)
            
        dft_feature = torch.cat((torch.sin(x_proj), torch.cos(x_proj)),dim=1) 
        dft_feature = dft_feature.permute(0, 1, 3, 2).contiguous().view(b, 2 * c, h, w)
