...
def chop_forward_dim4(t_im1, model, shave=8, scale=4, min_size=160000):
...
def multi_chop_forward_dim5(t_im1, model, shave=8, min_size=160000):
    b, t, c, h, w = t_im1.size()
    h_half, w_half = h // 2, w // 2
    h_size, w_size = h_half + shave, w_half + shave
    ...
    if w_size * h_size < min_size:
        ...
        for i in range(4):
            with torch.no_grad():
                input_batch = inputlist[i].clone()  # torch.cat(inputlist[i:(i + nGPUs)], dim=0)
                output_batch, flow = model(input_batch)
                if isinstance(output_batch, tuple):
                    output_batch = output_batch[0]
    ...
    with torch.no_grad(): 
        output_ht = Variable(t_im1.data.new(b, t, c, h, w))
        output_flow = Variable(t_im1.data.new(b, 2, c, h, w))
